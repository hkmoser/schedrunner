...
#         →  exportArchive (development method) → signed Dashboard.ipa + manifest.plist
...
# stable), display name ("Dashboard Next"), server port (:8443, via gen_ios_config →
...
APP_TITLE="Dashboard"; [[ "$DASHBOARD_CHANNEL" != "stable" ]] && APP_TITLE="Dashboard Next"

XCCONFIG="$REPO_ROOT/Config.xcconfig"
BUILD_DIR="$REPO_ROOT/build"
ARCHIVE="$BUILD_DIR/Dashboard.xcarchive"
...
PROJECT="$REPO_ROOT/Dashboard.xcodeproj"
...
if [[ "$(basename "$IPA")" != "Dashboard.ipa" ]]; then
  mv -f "$IPA" "$IPA_DIR/Dashboard.ipa"; IPA="$IPA_DIR/Dashboard.ipa"
fi
...
<plist version="1.0"><dict><key>items</key><array><dict>
  <key>assets</key><array>
    <dict><key>kind</key><string>software-package</string>
          <key>url</key><string>https://$TS_HOST$OTA_SFX/app/Dashboard.ipa</string></dict>
...
# All three artifacts (Dashboard.ipa, manifest.plist, index.html) now live in build/ipa/,
...
if have xcrun; then
  ...
  if [[ -n "$udid_now" ]]; then
    ...
    if xcrun devicectl device install app --device "$udid_now" "$IPA" 2>&1 | tee "$BUILD_DIR/devicectl-install.log"; then
      ...
      ok "installed Dashboard directly to the device — look for it on the home screen"
    else
