...
/// Dashboard banner — a 0-or-1 `repoBanner.cards` list, present when the specific repo has an
/// open PR or unmerged branch. Cached (ttl) so it doesn't hit GitHub on every dashboard load.
...
public struct LogsProvider: DataProvider {
    ...
    public func stub(config: AppConfig) -> JSONValue {
        ...
        return .obj([
            ...
            ("subtitleFormatted", .string("sidecar unavailable · sample data")),
            ("files", .array([
                file("dashboard.log", "12 KB · just now",
    ...
    public func stub(config: AppConfig) -> JSONValue {
        ...
        return .obj([
            ...
            ("repos", .array([
                ...
                repo("dashboard", "main", "2 uncommitted", "down", "uncommitted changes", "down", "10m ago · wip"),
