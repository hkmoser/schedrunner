...
enum RepoWatcher {
    ...
    static func start(on app: Application) {
        let interval = app.dashboardConfig.repoWatchInterval
            ...
            while !Task.isCancelled {
                ...
                if !ids.isEmpty {
                    ...
                    if let prior {
                        ...
                        if !newly.isEmpty && app.dashboardConfig.notifyEnabled && app.dashboardConfig.notifyRepos {
    ...
    private static func currentOpen(app: Application) async -> [Item] {
        let url = "\(app.dashboardConfig.bqSidecarURL)/repos"
