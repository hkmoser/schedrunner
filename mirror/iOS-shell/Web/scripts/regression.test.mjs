...
const sampleFor = (p) => (p === "/dashboard" ? "dashboard" : p.replace(/^\/screen\//, "").split("?")[0]) + "-sample.json";
...
// Routine background prefetch must target the dashboard + every nav leaf (so the latest
// of everything is cached for offline), minus the page already on screen.
const everyPage = new Set(["/dashboard", ...leaves.map((l) => l.path)]);
const targets = prefetchTargets(golden.nav, "/dashboard", "/screen/afm");
ok(targets.includes("/dashboard"), "prefetch includes the dashboard");
...
ok(!prefetchTargets(golden.nav, "/dashboard", "/screen/smarthome?types=light").includes("/screen/smarthome"),
...
const withExtra = prefetchTargets(golden.nav, "/dashboard", "/screen/logs", cached);
...
let hard = 0;
const on = buildDrawer(golden.nav, "/dashboard", () => {}, () => {}, { run: () => { hard++; }, enabled: true }).drawer;
...
const off = buildDrawer(golden.nav, "/dashboard", () => {}, () => {}, { run: () => { hard++; }, enabled: false }).drawer;
...
const ver = buildDrawer(golden.nav, "/dashboard", () => {}, () => {}, { run() {}, enabled: true }, "app abc · server abc", "Built Jun 20, 2026 · 3:45 PM").drawer;
...
let prefetchRan = 0;
const pfDrawer = buildDrawer(golden.nav, "/dashboard", () => {}, () => {}, { run() {}, enabled: true }, "app abc", "Built …",
...
const pfRunning = buildDrawer(golden.nav, "/dashboard", () => {}, () => {}, { run() {}, enabled: true }, "app abc", "Built …",
...
ok((renderScreen(decoy.screen, { data: decoy.data, theme: decoy.theme }, stubActions()).textContent ?? "").includes("Dashboard"), "decoy renders a dummy dashboard");
