...
// offline (not just the pages you happen to have opened): the dashboard plus every nav
...
export function prefetchTargets(
  ...
  // dashboard + every nav leaf + every already-cached page (incl. parameterized
...
// Every internal page link (`/screen/…` or `/dashboard…`) found anywhere in a fetched
...
export function collectInternalLinks(data: unknown): string[] {
  ...
  const visit = (v: unknown) => {
    if (typeof v === "string") {
      if (/^\/(screen\/|dashboard)/.test(v)) out.push(v);
