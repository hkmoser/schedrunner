...
// PWA — we brand it ("Dashboard Next", distinct theme) so the two home-screen apps are tellable
...
const PWA_NAME = IS_NEXT ? "Dashboard Next" : "Dashboard";
...
export default defineConfig({
  ...
  server: {
    ...
    proxy: {
      "/dashboard": {
        ...
        // The "/dashboard" key is a PREFIX match, so it also catches the static dev
        // fixture "/dashboard-sample.json". Let Vite serve any *-sample.json file itself
  ...
  plugins: [
    ...
    // apps would be labeled "Dashboard" and be indistinguishable. "Next" keeps the label short
    // enough to show in full under the icon.
    {
      ...
      transformIndexHtml(html: string) {
        ...
        return html
          .replace('<meta name="apple-mobile-web-app-title" content="Dashboard" />',
                   '<meta name="apple-mobile-web-app-title" content="Next" />')
          .replace("<title>Dashboard</title>", "<title>Dashboard Next</title>");
      },
    },
    VitePWA({
      ...
      manifest: {
        name: PWA_NAME,
        short_name: PWA_NAME,
        description: "Personal server-driven dashboard",
        display: "standalone",
      ...
      workbox: {
        ...
        // must reach the server, not the dashboard shell, even when the PWA SW is installed.
