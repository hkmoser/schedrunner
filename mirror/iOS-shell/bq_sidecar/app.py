"""BigQuery sidecar for the dashboard.
...
Config via environment:
  ...
  BQ_CONFIG_TABLE   override config table id (default: {BQ_DATASET}.dashboard_config)
  BQ_CONFIG_CREATE  "1" to create the config table if missing (default: 1)
  BQ_BALANCES_TABLE balances snapshot table (default: {BQ_DATASET}.dashboard_balances)
...
def _config_table():
    return os.environ.get("BQ_CONFIG_TABLE") or f"{_dataset()}.dashboard_config"
...
def _balances_table():
    return os.environ.get("BQ_BALANCES_TABLE") or f"{_dataset()}.dashboard_balances"
...
def _ci_status(repo, now):
    ...
    try:
        ...
        req = _urlreq.Request(url, headers={
            ...
            "User-Agent": "ios-shell-dashboard",
        })
...
def _self_repo_slug():
    """Repo to surface on the dashboard banner: DASHBOARD_PR_REPO (owner/name), else the git
...
def get_repo_banner():
    """A 0-or-1 `cards` list for the dashboard banner — present when the specific repo has an
...
def _docs_mirror():
    return os.path.expanduser(os.environ.get("DOCS_MIRROR") or "~/.cache/dashboard/docs")
...
SECRETS_FILE = os.path.expanduser(os.environ.get("SECRETS_FILE") or "~/.config/dashboard/secrets.json")
SECRETS_SM_NAME = os.environ.get("SECRETS_SM_NAME", "dashboard-secrets")
...
SETTINGS_SCHEMA = [
    ...
    # Easiest of all: drop the AuthKey_<ID>.p8 file in ~/.config/dashboard/apns and skip both.
