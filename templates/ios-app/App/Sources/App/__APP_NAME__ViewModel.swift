...
@MainActor
final class __APP_NAME__ViewModel: ObservableObject {
    @Published var manifest: Manifest?            // the main __APP_NAME_LOWER__
    @Published var currentPath: String = "/__APP_NAME_LOWER__"
    ...
    /// All-even passcode → a single decoy __APP_NAME_LOWER__ with dummy content, no menu.
    ...
    func bootstrap() {
        ...
        // Rehydrate every cached page (__APP_NAME_LOWER__ + visited/prefetched tabs) so the whole
        ...
        for cached in cache.loadAll() {
            ...
            if cached.path == "/__APP_NAME_LOWER__" { apply(cached.manifest) } else { pages[cached.path] = cached.manifest }
        }
    ...
    var current: Manifest? {
        currentPath == "/__APP_NAME_LOWER__" ? manifest : pages[currentPath]
    }
    ...
    func navigate(_ target: String) {
        ...
        if target == "back" { currentPath = "/__APP_NAME_LOWER__" } else { selectTab(target) }
    }
    ...
    private func load(_ path: String) async {
        do {
            ...
            if path == "/__APP_NAME_LOWER__" { apply(m) } else { pages[path] = m }
            cache.save(path, m)                    // persist EVERY page, not just the __APP_NAME_LOWER__
            pageFetchedAt[path] = Date()
            isOffline = false
            // After the __APP_NAME_LOWER__ lands, warm the rest of the nav once so tabs are instant.
            if path == "/__APP_NAME_LOWER__" && !didPrefetch { didPrefetch = true; prefetchNavPages() }
    ...
    private func liveActivityState(from m: Manifest) -> __APP_NAME__ActivityAttributes.ContentState {
        let cur = m.data?["bq"]?["currentState"]
        return __APP_NAME__ActivityAttributes.ContentState(
    ...
    func toggleLiveActivity() {
        ...
        guard ActivityManager.shared.areActivitiesEnabled else {
            liveActivityMessage = "Live Activities are turned off for __APP_NAME__. Enable them in "
                + "Settings → __APP_NAME__ → Live Activities (and make sure Settings → Face ID & "
        ...
        func begin(_ m: Manifest?) {
            ...
            case .notEnabled:
                liveActivityMessage = "Live Activities are turned off for __APP_NAME__ (Settings → __APP_NAME__)."
            case .failed(let err):
    ...
    private func prefetchNavPages() {
        ...
        func walk(_ items: [NavItem]?) {
            for it in items ?? [] {
                if let p = it.path, p != "/__APP_NAME_LOWER__" { paths.append(p) }
                walk(it.children)
            }
        }
        walk(nav)
    ...
    /// All nav destinations (+ the __APP_NAME_LOWER__), in order, for a full cache sweep.
    private func allNavPaths() -> [String] {
        var paths = ["/__APP_NAME_LOWER__"]
        func walk(_ items: [NavItem]?) {
            for it in items ?? [] {
                if let p = it.path, p != "/__APP_NAME_LOWER__" { paths.append(p) }
                walk(it.children)
    ...
    func cacheAllPages() {
        ...
        Task {
            for p in paths {
                if let m = try? await client.fetchPage(p) {
                    if p == "/__APP_NAME_LOWER__" { apply(m) } else { pages[p] = m }
                    cache.save(p, m)
                    pageFetchedAt[p] = Date()
                }
                prefetchDone += 1
    ...
    /// "Hard refresh": clear the on-disk cache and re-pull the __APP_NAME_LOWER__ + active page.
    ...
    func hardRefresh() {
        ...
        Task {
            await load("/__APP_NAME_LOWER__")
            if currentPath != "/__APP_NAME_LOWER__" { await load(currentPath) }
        }
    }
