...
/// works fully offline — not just the main __APP_NAME_LOWER__ but EVERY page that's been visited or
...
struct CacheStore {
    ...
    /// Save any page by its path. The __APP_NAME_LOWER__ is just path "/__APP_NAME_LOWER__".
    ...
    /// Convenience for the main __APP_NAME_LOWER__ (kept so BackgroundRefresh stays unchanged).
    func save(_ manifest: Manifest) { save("/__APP_NAME_LOWER__", manifest) }
    ...
    /// Convenience for the main __APP_NAME_LOWER__.
    func load() -> Cached? { load("/__APP_NAME_LOWER__") }
