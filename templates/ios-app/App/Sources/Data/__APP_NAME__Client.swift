...
/// (__APP_NAME__Host/__APP_NAME__Port), set via Config.xcconfig — a next-channel build talks to
/// :8443 instead of the default :443.
struct __APP_NAME__Client {
    let host: String
    let port: Int?   // nil = default 443

    init() {
        self.host = (Bundle.main.object(forInfoDictionaryKey: "__APP_NAME__Host") as? String) ?? ""
        let p = (Bundle.main.object(forInfoDictionaryKey: "__APP_NAME__Port") as? String).flatMap(Int.init)
    ...
    /// Main __APP_NAME_LOWER__. Sends the device timezone so times render in the user's zone.
    func fetch() async throws -> Manifest {
        try await fetchPage("/__APP_NAME_LOWER__")
    }
