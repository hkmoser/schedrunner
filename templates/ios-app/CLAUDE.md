...
- **stable** = branch **`main`**, this checkout, server `:8080` / sidecar `:8099` / `https://<TS_HOST>/`,
  app "__APP_NAME__". Deployed automatically by the external pull + `.auto-deploy` hook. This is the
...
id `<id>.next` (installs ALONGSIDE stable), name "__APP_NAME__ Next", talks to `:8443`
(`DASHBOARD_PORT` via `gen_ios_config.sh` → Info.plist `__APP_NAME__Port`), and publishes its OTA
...
- `Server/` (Vapor): `Composer` = templates + provider data; `Providers/` (Open-Meteo, Twelve Data,
  ...
  the board); serves `/__APP_NAME_LOWER__`, the navigable `/screen/bigquery` page, `/healthz`, and the web bundle.
...
- `bq_sidecar/` (Python): tiny localhost service over BigQuery via the machine's Application Default
  ...
  `/config` GET/POST (key/value `__APP_NAME_LOWER___config`
  ...
  `BQ_DATASET`/AFM; `BQ_YNAB_TABLE` overrides], falling back to the budget-sheet `__APP_NAME_LOWER___balances`
...
- Pages/navigation: every manifest carries `nav`, a **two-level tree** rendered as a **slide-out
  ...
  with SF Symbols). Destinations = Home `/__APP_NAME_LOWER__`; **Location** › Activity `/screen/afm` + Last 48
  ...
  **Offline** status and a refresh button; the drawer footer has **Hard refresh** (online-only — disabled
  ...
  digits → a **decoy __APP_NAME_LOWER__** rendered locally with dummy content only (restricted: menu hidden, no
...
- `App/` (SwiftUI): `App/Sources/SDUI/*` mirrors `Web/src/sdui/*` 1:1; adds private on-device
  ...
  fetched page (not just `/__APP_NAME_LOWER__`) keyed by a stable FNV-1a hash and prefetches the nav once
  per launch, so the whole app is browsable offline (`CacheStore`/`__APP_NAME__ViewModel`).
...
- After deploy: `curl https://<TS_HOST>/healthz` and `/__APP_NAME_LOWER__` (or `make doctor`).
