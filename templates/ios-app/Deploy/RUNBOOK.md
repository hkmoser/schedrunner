...
Turn-key deploy of the web __APP_NAME_LOWER__ over Tailscale HTTPS. The native iOS app is a
...
4. **setup_server** — `npm run build` (web) + `swift build -c release` (server), disable sleep,
   install a **KeepAlive LaunchAgent** (logs to `~/Library/Logs/__APP_NAME_LOWER__/`).
5. **healthcheck** — polls `/healthz`, validates `/__APP_NAME_LOWER__` (schema v1 + all cards), confirms `/` serves the web app.

## Day-2 operations
- `make doctor` — health probe anytime the __APP_NAME_LOWER__ looks wrong.
...
1. the budget-sheet `__APP_NAME_LOWER___balances` snapshot (set up via `Deploy/balances_sync.gs` — paste into the
...
- The app opens to a **4-digit passcode**: `1937` unlocks the full app; an all-even code (e.g. `2468`)
  opens a **decoy __APP_NAME_LOWER__** — a normal-looking __APP_NAME_LOWER__ rendered locally with dummy content, no
...
- **Docs** (`/screen/docs`) is a Markdown browser over Google Drive `/Private` (`DOCS_DIR`, auto-detected
  ...
  Markdown tree into a local dir (`DOCS_MIRROR`, default `~/.cache/__APP_NAME_LOWER__/docs`) — rsync materializes
...
| launchd labels | `__BUNDLE_ID__.*` | `…*-next` |
| URL | `https://$TS_HOST/` | `https://$TS_HOST:8443/` |
| Home-screen app | "__APP_NAME__" | "__APP_NAME__ Next" (purple, "NEXT" in the footer) |
...
- **Shared on purpose** (both channels are the same person's data on one Mac): BigQuery tables
  (`__APP_NAME_LOWER___config`, `known_locs`, balances/budget), the Drive mirrors, and the **Settings/
...
- **Native iOS OTA is channel-aware**: `make ios-deploy` (or the DEPLOY_IOS auto-build) in the
  next worktree produces a SEPARATE app — bundle id `<id>.next`, named "__APP_NAME__ Next", talking
...
- Both home-screen apps share the same icon art for now; they're distinguished by label
  ("__APP_NAME__" vs "Next"/"__APP_NAME__ Next") and, inside the app, an always-visible purple
...
- *Server won't start* → `tail -f ~/Library/Logs/__APP_NAME_LOWER__/server.err.log`.
...
The SwiftUI shell in `App/` renders the same `/__APP_NAME_LOWER__` manifest and adds the private on-device
...
`-allowProvisioningUpdates` registers the connected device), exports a signed `__APP_NAME__.ipa` +
`manifest.plist`, and publishes them + an install landing page to the server. No `Config.xcconfig`
edits required.

**Helpers / notes:**
- `make ios-generate` — just produce `__APP_NAME__.xcodeproj` to open in Xcode (e.g. to Run on the
