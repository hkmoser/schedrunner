 ...
 * BigQuery so the dashboard's Balances tab can read them.
 *
 ...
 *     `PROJECT.DATASET.dashboard_balances` (account STRING, balance FLOAT64, as_of TIMESTAMP)
 *
 * SETUP (one time):
...
var CONFIG = {
  ...
  TABLE: 'dashboard_balances',                    // snapshot table (created/replaced)
