...
app_setting() {
  ...
  if [[ -z "$v" ]]; then
    local f="${SECRETS_FILE:-$HOME/.config/__APP_NAME_LOWER__/secrets.json}"
...
publish_app_bundle() {
  ...
  [[ -f "$src/__APP_NAME__.ipa" ]] || return 0     # nothing built yet → nothing to publish
  local dest="$web_dist/app"
  mkdir -p "$dest" || return 0
  local f
  for f in __APP_NAME__.ipa manifest.plist index.html version.json; do
...
reload_service() {
  ...
  if launchctl bootstrap "$domain" "$PLIST" 2>/tmp/__APP_NAME_LOWER__-launchctl.err; then
    ...
    launchctl load -w "$PLIST" 2>/dev/null \
      || fail "launchctl could not load the service: $(cat /tmp/__APP_NAME_LOWER__-launchctl.err 2>/dev/null)"
