...
if ! have gcloud; then
  ...
  info "0600 local file (~/.config/__APP_NAME_LOWER__/secrets.json). Install the Google Cloud SDK to use Secret Manager."
  exit 0
fi
...
if gcloud services enable secretmanager.googleapis.com --project "$PROJECT" 2>/tmp/__APP_NAME_LOWER__-sm.err; then
  ok "Secret Manager API enabled"
else
  warn "couldn't enable the Secret Manager API:"
  info "$(cat /tmp/__APP_NAME_LOWER__-sm.err 2>/dev/null)"
...
if [[ -n "$ACCOUNT" && "$ACCOUNT" != "(unset)" ]]; then
  ...
  if gcloud projects add-iam-policy-binding "$PROJECT" \
        ...
        --condition=None >/dev/null 2>/tmp/__APP_NAME_LOWER__-iam.err; then
    ...
    info "$(head -n1 /tmp/__APP_NAME_LOWER__-iam.err 2>/dev/null)"
  fi
fi
