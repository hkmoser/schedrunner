...
[[ $ok_health -eq 1 ]] || fail "server did not become healthy; check: tail -f \"$HOME/Library/Logs/__APP_NAME_LOWER__/server$CHANNEL_SUFFIX.err.log\""
ok "health endpoint responding ($url_base/healthz)"

# Validate the manifest is well-formed and complete (schema v1 + all cards).
manifest_json="$(curl -fsS --max-time 10 "$url_base/__APP_NAME_LOWER__")" || fail "/__APP_NAME_LOWER__ did not respond"
...
app_name="__APP_NAME__"; [[ "$DASHBOARD_CHANNEL" == "next" ]] && app_name="__APP_NAME__ Next"
