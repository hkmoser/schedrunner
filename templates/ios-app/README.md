# iOS-Shell — Personal Server-Driven __APP_NAME__

A personal __APP_NAME_LOWER__ built as a **server-driven UI (SDUI)** shell. A small Swift/Vapor service on
...
cd Web && npm install && npm run dev   # Vite dev server, proxies /__APP_NAME_LOWER__ to :8080
```

## Layout
- `Server/` — Vapor: data providers, manifest composer, serves the web bundle + `/__APP_NAME_LOWER__`.
