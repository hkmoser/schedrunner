...
public struct APNsConfig: Sendable {
    ...
    ///            dropped in ~/.config/__APP_NAME_LOWER__/apns (or APNS_KEY_DIR).
    ///  - key id: APNS_KEY_ID → else parsed from the AuthKey_<ID>.p8 filename.
    ///  - team:   APNS_TEAM_ID → else the iOS build's DEVELOPMENT_TEAM.
    ///  - bundle: APNS_BUNDLE_ID → else PRODUCT_BUNDLE_IDENTIFIER → else __BUNDLE_ID__.
    ...
    public static func load() -> APNsConfig? {
        ...
        let bundleID = env("APNS_BUNDLE_ID") ?? env("PRODUCT_BUNDLE_IDENTIFIER") ?? "__BUNDLE_ID__"
    ...
    private static func findAuthKey() -> String? {
        ...
        let dirs = [Environment.get("APNS_KEY_DIR"),
                    "\(home)/.config/__APP_NAME_LOWER__/apns",
                    "\(home)/.config/__APP_NAME_LOWER__"].compactMap { $0 }
        for dir in dirs {
    ...
    public static func diagnose() -> String {
        ...
        if let inline = env("APNS_KEY_P8") {
            ...
            parts.append("key=✗ NONE (paste APNS_KEY_P8 on Settings, or drop AuthKey_*.p8 in ~/.config/__APP_NAME_LOWER__/apns)")
        }
