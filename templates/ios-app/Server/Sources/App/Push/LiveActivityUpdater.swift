...
enum LiveActivityUpdater {
    ...
    static func start(on app: Application) {
        let interval = app.__APP_NAME_LOWER__Config.liveActivityInterval
        guard interval > 0 else { return }
        let dismissOn = app.__APP_NAME_LOWER__Config.liveActivityDismissOn.lowercased()
    ...
    private static func currentState(app: Application) async -> AFMState? {
        let url = "\(app.__APP_NAME_LOWER__Config.bqSidecarURL)/afm"
    ...
    /// keys match __APP_NAME__ActivityAttributes.ContentState exactly.
