...
enum LogFailureWatcher {
    ...
    static func start(on app: Application, config: APNsConfig) {
        let interval = max(30, app.__APP_NAME_LOWER__Config.logWatchInterval)
        ...
        let task = Task {
            ...
            while !Task.isCancelled {
                ...
                if !seeded {
                    ...
                    if !newly.isEmpty && app.__APP_NAME_LOWER__Config.notifyEnabled && app.__APP_NAME_LOWER__Config.notifyLogFailures {
    ...
    private static func currentFailures(app: Application) async -> Set<String> {
        let url = "\(app.__APP_NAME_LOWER__Config.bqSidecarURL)/logs"
