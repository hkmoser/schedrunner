...
public func configure(_ app: Application) async throws {
    ...
    app.__APP_NAME_LOWER__Config = AppConfig.load(app.environment)
    ...
    // router, so /__APP_NAME_LOWER__ and /healthz are unaffected.
    if var dist = app.__APP_NAME_LOWER__Config.webDist {
    ...
    if app.environment != .testing && app.__APP_NAME_LOWER__Config.warmEnabled {
        Warmer.start(on: app)
    }
