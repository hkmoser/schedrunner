...
for (const p of ["/dashboard", "/screen/afm", "/screen/afm_log", "/screen/afm48", "/screen/afm_health", "/screen/balances", "/screen/budget", "/screen/bqtables", "/screen/smarthome", "/screen/smarthome_log", "/screen/messages", "/screen/logs", "/screen/repos", "/screen/schedrunner", "/screen/schedlogs", "/screen/deploy", "/screen/docs", "/screen/config", "/screen/settings"]) {
...
let hardRefreshed = 0;
const { drawer: drawer2 } = buildDrawer(golden.nav, "/dashboard", () => {}, () => {}, { run: () => { hardRefreshed++; }, enabled: true });
...
const { drawer: drawer3 } = buildDrawer(golden.nav, "/dashboard", () => {}, () => {}, { run: () => { hardRefreshed2++; }, enabled: false });
...
const { drawer: drawer4 } = buildDrawer(golden.nav, "/dashboard", () => {}, () => {}, { run: () => {}, enabled: true }, "app abc1234 · server abc1234");
...
assert.equal(classifyPasscode("2468"), "decoy", "all-even opens the decoy dashboard");
assert.equal(classifyPasscode("0000"), "decoy", "all-even (zeros) opens the decoy dashboard");
...
// Decoy dashboard (all-even passcode): a dashboard-shaped page of dummy content,
...
assert.ok(decoyText.includes("Dashboard"), "decoy shows a dashboard title");
...
assert.ok(logsText.includes("dashboard.log"), "a log file name renders");
