...
const PREFIX = "__APP_NAME_LOWER__.page.v1:";
const LEGACY_KEY = "__APP_NAME_LOWER__.manifest.v1"; // earlier __APP_NAME_LOWER__-only cache
...
// Per-page cache so every tab (__APP_NAME_LOWER__, Activity, Config) paints instantly from
...
export function loadCached(path = "/__APP_NAME_LOWER__"): CachedManifest | null {
  try {
    const raw = localStorage.getItem(PREFIX + path) ?? (path === "/__APP_NAME_LOWER__" ? localStorage.getItem(LEGACY_KEY) : null);
...
export function saveCached(manifest: Manifest, path = "/__APP_NAME_LOWER__"): void {
  try {
...
export function cachedPagePaths(): string[] {
  ...
  try {
    for (let i = 0; i < localStorage.length; i++) {
      ...
      else if (k === LEGACY_KEY) out.push("/__APP_NAME_LOWER__");
    }
  } catch {
    /* non-fatal */
  }
