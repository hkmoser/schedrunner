...
// reliably than server-side IP geolocation, so the client sends them to /__APP_NAME_LOWER__.
...
/** Build the query string sent to /__APP_NAME_LOWER__ from device tz + cached coords, honoring
 ...
 * invokes `onUpdate` so the __APP_NAME_LOWER__ can refresh with the precise location.
 */
