...
// Set by the passcode gate: "decoy" shows a single dummy __APP_NAME_LOWER__ with no menu
// (all-even code); "full" shows the whole app (1937).
let restricted = false;
const MAIN = "/__APP_NAME_LOWER__";
...
const actions: ActionContext = {
  ...
  setPref: (key, value) => {
    ...
    // The weather Home/Current toggle changes the __APP_NAME_LOWER__ query — re-pull so it applies.
    if (key === "weatherLoc") {
  ...
  if (!compat.ok) {
    showMessage(
      ...
      compat.reason === "tooNew"
        ? "This __APP_NAME_LOWER__ shell is older than the server. Update the app."
        : "The __APP_NAME_LOWER__ data was malformed.",
    );
    return;
  }
  ...
  try {
    ...
    if (cachedPage || current.manifest) {
      ...
      showMessage(
        "Offline",
        path === MAIN
          ? "Can't reach the __APP_NAME_LOWER__ (is Tailscale on?) and there's nothing cached yet."
...
async function prefetchAll(deep = false): Promise<void> {
  ...
  const enqueue = (p: string) => {
    ...
    if (!/^\/(screen\/|__APP_NAME_LOWER__)/.test(p)) return; // internal pages only
...
function startApp() {
  // cache-first paint of the __APP_NAME_LOWER__, then revalidate.
  const cached = loadCached(MAIN);
  if (cached) {
    current.manifest = cached.manifest;
    render(cached.manifest);
  } else {
    showMessage("Loading…", "Fetching your __APP_NAME_LOWER__.");
  }
  void selectTab(MAIN);

  // Prime the offline cache shortly after boot (let the __APP_NAME_LOWER__ paint first): a DEEP
...
// Decoy (all-even passcode): render a single dummy __APP_NAME_LOWER__ locally — no network,
